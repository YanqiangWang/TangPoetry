...
import com.jkxy.poetry.util.ListUtil;
import com.jkxy.poetry.util.PageUtil;
import com.jkxy.poetry.util.StringUtil;
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (poetList.size() > 0 && poetryList.size() > 0)
			...
			if (poetItemId > 0 && poetryItemId > 0) {
				poetryContent = poetryDao.getContentByTitle(poetList.get(
						poetItemId - 1).getId(), poetryList
						.get(poetryItemId - 1));
			}
		// 获取列表代码
		...
		// 获取分页选项卡代码
		String poetPageCode = PageUtil
				.genPagination("PoetPageServlet", totalPoetNum, poetPage,
						Constants.POET_PAGE_NUM, "currPoetPage");
		String poetryPageCode = PageUtil.genPagination("PoetryPageServlet",
				totalPoetryNum, poetryPage, Constants.POETRY_PAGE_NUM,
				"currPoetryPage");
		// 获得内容页面代码
		String contentCode = "";
		if (StringUtil.isNotEmpty(poetryContent)) {
			contentCode = PageUtil.genPagination(poetName, poetryName,
					poetryContent);
		} else {
			// 默认显示主页提示信息
